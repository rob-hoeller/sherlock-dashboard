...
interface Digest { date: string; path: string; size: number; }
...
export default function DigestsPage() {
  ...
  useEffect(() => {
    fetch("/api/digests").then((r) => r.json()).then((d: Digest[]) => {
      setDigests(d);
      if (d.length > 0) loadDigest(d[0].date);
    });
